...
(function () {
  ...
  // Reconstructs the full ΔT audit trail from project.result data.
  // Mirrors the Python-side audit trail format previously generated by the
  // pipeline and pre-baked into brief HTML.  Now generated on-demand in the
  // browser so on-the-fly what-if projects get the same detail as pipeline ones.
  function _buildAuditText(project, result, params) {
    const hz        = result.hazard_zone  || 'non_fhsz';
    const ut        = +(params.unit_threshold    || 15);
    const vpu       = +(params.vehicles_per_unit || 2.5);
    const mob       = +(params.mobilization_rate || 0.90);
    const maxShare  = +(params.max_project_share || 0.05);
    const pv        = +(result.project_vehicles  || 0);
    const ep        = +(result.egress_minutes    || 0);

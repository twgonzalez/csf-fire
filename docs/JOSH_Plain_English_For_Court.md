...
Not everyone drives out in an evacuation. Some people shelter in place. Some ride with neighbors. Researchers at the University of Florida and UC Berkeley tracked millions of cell phones during actual California wildfires — the 2019 Kincade Fire in Sonoma County, the 2017 Tubbs Fire, and others — and measured what fraction of households actually drove out. The answer depends on how close you are to the danger. In the highest-hazard fire zones, about 75% of households evacuate by car. In areas farther from the fire, about 25% do.

So for a proposed project of 45 homes in a high-hazard fire zone:

```
45 homes × 2.5 vehicles × 75% who drive out = 84 vehicles
```

That is how many cars this project puts on the road during an evacuation.
...
But here is the critical point: if the road runs through the same fire zone that is causing the evacuation, it will not work at full capacity. There will be smoke reducing visibility. There will be fire trucks driving in the opposite direction. There may be downed trees or power lines blocking a lane. The Highway Capacity Manual publishes adjustment factors for exactly these conditions — reduced visibility, lane blockages, incidents. The state of California, through Cal Fire, publishes maps showing which areas will experience severe fire behavior. These are called Fire Hazard Severity Zone designations, and they are made under state law based on fuel, terrain, and weather data.
...
```
84 vehicles ÷ 472 vehicles per hour = 0.178 hours
0.178 hours × 60 = 10.7 minutes
Building egress penalty: 0 minutes (low-rise)
Total: 10.7 minutes
```

**This project adds 10.7 minutes to the time it takes everyone on that road to reach safety.**
...
The same logic applies here. The city sets a threshold for each type of hazard zone. In the most dangerous fire zones — where the time between the start of a fire and its arrival at a community can be as short as 30 to 45 minutes — the threshold is tighter, because the consequences of delay are more severe. In areas with no fire designation, the threshold is more generous, because there is more time.

| Zone | Threshold | How It Is Calculated |
|---|---|---|
| Very High Fire Hazard | 2.25 minutes | 45-minute escape window × 5% |
| High Fire Hazard | 4.5 minutes | 90-minute escape window × 5% |
| Moderate Fire Hazard | 6 minutes | 120-minute escape window × 5% |
| No fire designation | 6 minutes | 120-minute escape window × 5% |

The escape window — how long residents have from the first warning to when the fire makes the area impassable — comes from the most detailed wildfire investigation ever conducted. After the 2018 Camp Fire killed 85 people in Paradise, California, the National Institute of Standards and Technology spent years reconstructing a minute-by-minute timeline. They documented that in the highest-hazard zones, residents had approximately 45 minutes from the first indication of danger to the arrival of the fire front. That finding is NIST Technical Note 2135, a published federal research document.

The 5% figure means that no single project may consume more than one-twentieth of that window. At 5%, a road can absorb approximately 20 projects of similar size before new development alone would fill the entire escape window. The city chose a permissive standard — one that allows substantial new development — while ensuring no single project takes a disproportionate share of the time people need to escape.

The threshold values are not round numbers — 2.25 minutes, not 3; 4.50 minutes, not 5. This is intentional. A calculated result that traces to a published federal study multiplied by a standard engineering threshold is more defensible than a round number that appears chosen by staff. The math produces the number.
...
Our canyon road project adds 10.7 minutes in a Very High zone with a 2.25-minute threshold. It exceeds the threshold by 8.45 minutes. Additional review is required.

A different project — 15 homes on a four-lane road outside any fire zone — adds 0.3 minutes against a 6-minute threshold. It passes easily. Approved.
...
The standard works the same way. It measures the project's vehicles against the road's physical capacity. That calculation produces the same number whether the road is underused or gridlocked. Adding 84 vehicles to a 472-vehicle-per-hour road consumes 10.7 minutes of that road's throughput — period. The existing population is not in the formula because it does not change the project's impact on the bottleneck.

This means the standard does not prohibit building in areas that are already crowded. It limits the size of what any single project can add, proportional to the road that serves it. A 12-unit project on that same canyon road produces about 2.2 minutes — under the threshold. Approved. The developer can build. But no single project gets to consume more than 2.25 minutes of the bottleneck's time — which is 5% of the 45-minute safe egress window established by the NIST Camp Fire case study.
...
**Build fewer units.** Fewer homes means fewer cars. Our 45-unit project could be reduced to about 12 units to bring the time contribution below 3 minutes on that canyon road.
...
```
Our project needs the bottleneck to handle 84 vehicles in 2.25 minutes.
That requires: (84 ÷ 2.25) × 60 = 2,240 vehicles per hour.
The road currently handles 472 vehicles per hour.
The gap is 1,768 vehicles per hour.
```
...
| Fraction of households that evacuate by car | GPS analysis of 44.2 million cell phone records during the 2019 Kincade Fire | Zhao et al. (2022), published in Transportation Research Part C (peer-reviewed) |
| Cross-validation of evacuation rates | Survey of 589 evacuees from three California wildfires (2017-2019) | Wong, Broader & Shaheen (2020), UC Institute of Transportation Studies |
...
1. **Count the cars** the project creates (Census data × empirical evacuation rates from California wildfires).
...
4. **Divide** to get minutes. Compare to the threshold for the hazard zone.
